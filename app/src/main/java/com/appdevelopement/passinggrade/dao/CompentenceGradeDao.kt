...
interface CompentenceGradeDao {
    ...
    @Query("SELECT * FROM CompetenceGrade WHERE studentNumber = :studentId AND (SELECT idComptence FROM Compentence WHERE idExam = :idExam) ORDER BY idCompetenceGradeId DESC LIMIT (SELECT COUNT(idExam) FROM Compentence WHERE idExam = :idExam)")
