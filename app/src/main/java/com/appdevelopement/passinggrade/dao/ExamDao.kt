...
@Dao
interface ExamDao {
  @Query("SELECT * FROM Exam") fun getAll(): List<Exam>

  @Insert(onConflict = OnConflictStrategy.REPLACE) fun insertExam(exam: Exam): Long

  @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertexam(exam: Exam)
