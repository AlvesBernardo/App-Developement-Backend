...
object TeacherManager {
  ...
  fun addTeacher(context: Context) {
    ...
    CoroutineScope(IO).launch { dao.insertTeacher(teacher2) }
  }

  suspend fun getTeacherByCredentials(
      context: Context,
      username: String,
      password: String
  ): Teacher? {
    val dao = AppDatabase.getDatabase(context).teacherDao()
    return withContext(Dispatchers.IO) { dao.getTeacherByCredentials(username, password) }
  }
}
